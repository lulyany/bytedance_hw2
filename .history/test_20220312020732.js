...
function init(){  //初始化
  ...
  Snak_head_svg.style.top = coordinate_Y[0] - 0 + "px"
  Snak_head_svg   .style.left = coordinate_X[0] + 0 + "px"
  let node = map.children
  for (let i = node.length - 1; i >= 0; i--) {
    if (node[i].className === "Snak_bo dy") map.removeChild(node[i])
  }
}
